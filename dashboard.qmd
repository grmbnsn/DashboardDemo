...
```{python}
#| fig-cap: A line chart showing spending by geography over time
#| title: Annual Per Capita Award Amount
px.line(df, x='Year', y='Per Capita', color='District')
```
...
```{python}
#| label: fig-bar1
#| fig-cap: A stacked bar chart showing spending by geography over time
...
```{python}
#| label: fig-bar2
#| fig-cap: A grouped bar chart showing spending by geography over time
